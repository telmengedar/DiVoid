...
- **Fail-closed auth** — no usable credential source (neither environment variable set and no fallback file), or an environment that is only partially set, exits the process before the stdio loop. Narrowed by DiVoid #13108, which is authoritative for credential resolution; node #695 carries the pre-Phase-1-credential-contract wording and is reconciled separately.
