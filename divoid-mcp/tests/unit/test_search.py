...
def server() -> FastMCP:
    """Module-scoped FastMCP server with only divoid_search registered."""
    config = DivoidConfig(base_url=_DUMMY_BASE, api_key=_DUMMY_KEY)
    http_client.init(_DUMMY_BASE, _DUMMY_KEY)

    mcp_server = FastMCP("divoid-mcp-search-test")
...
async def _call(server: FastMCP, args: dict[str, Any]) -> dict[str, Any]:
    """Call divoid_search with the given args and return the raw dict."""
    result = await server._tool_manager.call_tool("divoid_search", args)
...
async def test_flag_on_appends_field_and_normalizes_output(server: FastMCP) -> None:
    ...
    result row carries normalized link_details.
    """
    payload = {
        "result": [
            {
...
async def test_composes_with_include_links(server: FastMCP) -> None:
    ...
    'linkDetails' to fields; the row carries both keys.
    """
    payload = {
        "result": [
            {
