"""Unit tests for divoid_create_session_log's root_node_id defaulting and result
reporting. DiVoid #10986 / #11284."""
...
def server() -> FastMCP:
    ...
    mcp_server = FastMCP("divoid-mcp-create-session-log-test")
...
async def test_project_id_only_defaults_root_node_id(server: FastMCP) -> None:
    ...
    with respx.mock(assert_all_called=True) as mock:
        ...
        result = await _call(server, {
            "name": "Session-log with only project_id",
            "content": "narrative",
    ...
    assert result["docs_group_id"] == _DOCS_GROUP_ID
...
async def test_explicit_root_node_id_wins_over_project_id(server: FastMCP) -> None:
    ...
    with respx.mock(assert_all_called=True) as mock:
        ...
        result = await _call(server, {
            "name": "Session-log with differing explicit root_node_id",
            "content": "narrative",
            "project_id": _PROJECT_ID,
...
async def test_explicit_falsy_root_node_id_zero_wins_over_project_id(server: FastMCP) -> None:
    ...
    with respx.mock(assert_all_called=True) as mock:
        ...
        result = await _call(server, {
            "name": "Session-log with explicit root_node_id=0",
            "content": "narrative",
            "project_id": _PROJECT_ID,
            "root_node_id": 0,
        })
...
@pytest.mark.asyncio
async def test_docs_group_id_without_root_node_id_stays_null(server: FastMCP) -> None:
    captured: list[httpx.Request] = []
    ...
    with respx.mock(assert_all_called=True) as mock:
        ...
        result = await _call(server, {
            "name": "Session-log via docs_group_id, no root_node_id",
            "content": "narrative",
    ...
    assert "rootNodeId" not in body, (
        f"Expected no rootNodeId key on the POST body when only docs_group_id is given, "
        f"got: {body!r}"
    )
...
@pytest.mark.asyncio
async def test_docs_group_id_with_explicit_root_node_id_applies_it(server: FastMCP) -> None:
    captured: list[httpx.Request] = []
    explicit_root = 2
    ...
    with respx.mock(assert_all_called=True) as mock:
        ...
        result = await _call(server, {
            "name": "Session-log via docs_group_id with explicit root_node_id",
            "content": "narrative",
            "docs_group_id": _DOCS_GROUP_ID,
            "root_node_id": explicit_root,
        })
...
async def test_root_node_id_falls_back_when_response_omits_it(server: FastMCP) -> None:
    ...
    with respx.mock(assert_all_called=True) as mock:
        ...
        result = await _call(server, {
            "name": "Session-log, backend response omits rootNodeId",
            "content": "narrative",
            "project_id": _PROJECT_ID,
        })
...
async def test_server_echoed_root_node_id_wins_over_computed_value(server: FastMCP) -> None:
    ...
    with respx.mock(assert_all_called=True) as mock:
        ...
        result = await _call(server, {
            "name": "Session-log, server echoes a different rootNodeId than requested",
            "content": "narrative",
            "project_id": _PROJECT_ID,
        })
...
async def test_explicit_null_echo_is_honoured_as_honest_null(server: FastMCP) -> None:
    with respx.mock(assert_all_called=True) as mock:
        ...
        result = await _call(server, {
            "name": "Session-log, server explicitly echoes rootNodeId=null",
            "content": "narrative",
            "project_id": _PROJECT_ID,
        })
