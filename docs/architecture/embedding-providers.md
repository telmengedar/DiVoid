...
- **Composition policy is settled** and currently lives in *two* places: the C# `EmbeddingInputComposer.Compose(name, content, contentType)` (`Backend/Services/Embeddings/EmbeddingInputComposer.cs`) and the SQL `BuildEmbeddingBranchOperations` server-side branches (`NodeService.cs:996`). This design keeps both paths (Option D preserves the SQL path) and introduces a single constants holder both consume (§5.9, §6.7).
