...
4. **`total` count semantics.** Compute `total` with a parallel `SELECT COUNT(*)` over the same composed predicate (the existing `AsyncPageResponseWriter<T>` constructor takes a count delegate; reuse that). On large graphs this is the more expensive of the two queries; document that fact in the API reference. **Trade-off considered:** skipping count when `count` was not requested. Rejected — every existing list endpoint always returns `total` and clients depend on it. **Future hook:** an opt-out parameter (`?nototal=true`) can be added later without a redesign.
