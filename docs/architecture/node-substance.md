...
**Corroborated by two independent re-derivations, which matters because the whole change rests on this list being complete.** The implementer did not inherit this result: during PR 3 he ran `grep -rn "Update<Node>\|Insert<Node>" Backend/` himself, read each site's `.Set(…)` / `.Columns(…)`, and reached the same 17 sites with exactly two assigning `Content` — `NodeService.cs:1167` and `:1236` — and `CreateNode`'s INSERT column list not among them. Re-run once more against the tree with PR 3 applied: still 17 sites, still 2 assignments. **Three derivations, three agreements, none of them a reading of the other two.** An inventory claimed once is a single point of failure for every invariant built on it; this one is not that.
...
| C7 | `UploadContent_WhenEmbeddingRegenerationFails_LeavesSubstanceIntact` | **I5's atomicity half** | built like the shipped S9: seed with a `DisabledCapability` service, then call `UploadContent` on one constructed with an **enabled** capability against the same SQLite fixture — it reaches the Postgres-only `embedding` function and throws, so `transaction.Commit()` is never reached. **The seeded node must carry a name and text content type**, or `EmbeddingInputComposer.Compose` returns null, the `Embedding = null` branch runs cleanly on SQLite, nothing throws and the row proves nothing. The test asserts the throw **first** — that scenario check is what establishes the content UPDATE *executed*, since the throw comes from a helper ordered after it. **And the node is seeded with content `v1` first, so both assertions are on a surviving prior value, not on absence** — see the C7 note below for why that distinction is the whole row | **Two mutations, both executed and observed on the fixed test** — no predicted mutation appears in this cell (see the standard below). **M12**: drop `n => n.Content == blob` from the upload SET → `Expected: "# C7 v1 body that must survive the failed upload" / But was: null`. **R-pre**: relocate the clear to a standalone UPDATE *before* the transaction opens → `Expected: "C7|must-survive-rollback" / But was: null`, 138 ms, terminates. |
| C8 | `Patch_ReplaceContent_IsRejected` | **R7** — `PATCH /api/nodes/{id}` is not a content-write path, so §17.2's inventory is closed at two | asserts the **status code is exactly 400**. On the compliant path the `[AllowPatch]` check at `DatabasePatchExtensions.cs:47–48` runs *before* the value conversion at `:50–54`, so the patch value can be any string and the conversion is never reached. **That premise describes the compliant path only — see the note below for what the mutated path does.** The accompanying "content and substance unchanged" assertion is documentation, not the guard | `[AllowPatch]` is added to `Node.Content`: the PATCH returns **500** (measured during PR 3 implementation), and 500 ≠ 400 reddens the row exactly as a 200 would have |
...
**C8's mechanism, corrected 2026-09-05 after measurement — and the part that matters is not the number.** This row originally predicted the mutation would return **200**. It returns **500**. With `[AllowPatch]` present on `Node.Content`, execution no longer stops at the gate and reaches `Converter.Convert(<the string patch value>, typeof(byte[]))`, which faults; none of the eight registered `IErrorHandler`s maps a conversion fault (`PropertyNotFound`, `NotSupported`, `PathQueryParse`, `SemanticSearchUnavailable`, `UnknownField`, `Argument`, `AuthenticationFailed`, `AuthorizationFailed` — `Startup.cs:80–90`), so it surfaces as a 500. **The row is unaffected**: it asserts 400 exactly, and any other status reddens it.

**What the correction changes is a safety claim, not a test.** As written, the row implied the `[AllowPatch]` gate is the *only* thing standing between a caller and a successful content patch. It is not — the `byte[]` type conversion is a second barrier, and anyone auditing the `[AllowPatch]` list should know three things about it: it is **accidental**, not designed; it fails **loudly** rather than corrupting content, which is the good direction; and **it does not generalise**. It holds only because `Content` is `byte[]` and the patch value arrives as a string. An `[AllowPatch]` added by mistake to any *string*-typed property converts cleanly and would succeed silently. *Falsifier:* if `Converter.Convert` ever gains a string→`byte[]` path (base64, say), the mutation returns 200 again — the row still reddens, but this paragraph's second barrier disappears.


**C7's falsifier cell has been wrong three times, so the rule for it changed rather than the prediction.** Round 1 named *“issue the clear outside the transaction”* and promised it would redden by assertion or by a transaction error. Round 2 replaced it with *“hoist `transaction.Commit()` above the embedding call”*, argued to be runnable where the first was not. **Both were predictions, and both were wrong.** Measured by the implementer:

| Predicted mutation | What actually happened |
|---|---|
| relocate the clear outside the transaction | **equivalent mutant** on C7's fixture — passes in 114 ms. Not a kill, not a hang. See the substrate note below |
| hoist `Commit()`, reusing the transaction handle | reddens, but by an escaping `InvalidOperationException: The transaction object is not associated with the same connection object as this command` from `NodeService.cs:1195`. The test's `catch (StatementException)` does not catch it, so it escapes **before any assertion runs** — a kill, but not the value comparison the cell named, and not at assertion level |
| hoist `Commit()` plus a fresh transaction for the embedding — the realistic “split the transaction” refactor, and the thing this row is actually guarding against | **hangs.** Killed at a 10-minute timeout against a 69-second suite |

**And the earlier “deadlock” write-up in this document was itself wrong, which is worth correcting rather than deleting.** Round 2 recorded that the relocation hung for 12 minutes against a 73-second baseline. Reproduced under `--blame-hang`, **those 12 minutes came from HTTP tests in the same filter, not from C7.** Two people measured one row and got a red and a hang because they had built neighbouring mutations — one placed *before* the transaction opened, one *after*. Same idea, opposite sides of one line, entirely different outcomes.

**The real substrate hazard, which is worse than a hang because it is silent.** `DatabaseFixture` opens **one `SqliteConnection` for the lifetime of the fixture** and Ocelot reuses it for every operation (`Backend.tests/Fixtures/DatabaseFixture.cs`). So a statement issued *without* an explicit transaction handle is captured by whatever transaction is already open on that connection — and rolls back with it. **The obvious “move it outside the transaction” mutation is therefore an *equivalent mutant* here: it passes, and it looks like the guard holding.** A mutation that hangs at least announces itself; one that is silently equivalent reports green and is indistinguishable from a real pass.

**This is a known class on this substrate, not a novelty.** #1217's §4 addendum of 2026-08-21 records the identical mechanism from mamgo's settlement outbox (PR #852): single shared connection, `ExecuteAsync(transaction)` → `ExecuteAsync()` observably unchanged, only a full revert visible. Detail: **#8914**. Its guidance is what applies here — fall back to the coarsest mutation the substrate *can* observe, and say which one you used and which you could not. **R-pre is that fallback**: relocating the clear to *before* the transaction opens escapes the ambient capture and is observable, which is why it is the mutation in the cell and the relocation *after* it is not.

**M12 is also the proof the assertion remedy worked, measured on both sides.** The implementer reconstructed the pre-fix `Is.Null` form, applied M12, and it **passed** — M12 is literally the “nothing ever happened” world, so the seed never lands, content stays null, and `Is.Null` is satisfied. The seeded-survival form kills it. That is the absence-versus-value argument below, demonstrated rather than asserted.

### The standard for this row's falsifier cell, from 2026-09-05

> **A falsifier cell may only name a mutation whose observed output someone has quoted. If none exists, the cell says “no runnable falsifier established”.**

That is deliberately weaker than *“run it yourself”* — building a mutation harness is not this document's artifact and never was. It is also sufficient, because every failure above came from writing a cell with no observation behind it at all. **“No runnable falsifier established” is an honest, useful cell**, and it is what C7 should have carried two rounds ago; a table that can say so is strictly better than one that must always produce a name.

**And the author wrote this rule one round before breaking it.** The C8 correction closed with: *the separating question is not what the assertion observes, but whether anyone has executed the mutation and watched it go red.* The very next revision of this row named a hoist mutation nobody had run. **A rule stated in a document does not bind the next paragraph of the same document** — which is the argument for making it a constraint on the cell's *contents* rather than a principle in its prose, and that is what the standard above does.

**What the row can and cannot isolate — revised, because the measurements refute the bound this paragraph previously carried.** Two earlier revisions said *“no runnable mutation isolates the substance half alone, because inside the transaction is compliant and outside is equivalent-or-hanging”*. **R-pre refutes that.** Placed *before* `using Transaction transaction = database.Transaction()` (`NodeService.cs:1164`), the clear commits immediately, the content write then rolls back, and the final row is content `v1` / substance `null` — so the content assertion **passes** and only the substance assertion reddens. R-pre isolates the substance half exactly, in 138 ms.

**The real boundary is not inside-versus-outside; it is before-versus-after the transaction opens.**

| Placement of the clear | Observable on this fixture? |
|---|---|
| in the content UPDATE's own SET (shipped) | — compliant |
| its own statement, *inside* the transaction | **no** — also compliant; §17.5 says so, and no row pins “same statement” |
| its own statement, no handle, *after* the transaction opens | **no — equivalent mutant.** The ambient transaction on the shared connection captures it, so it rolls back too and the test is green |
| its own statement, *before* the transaction opens (**R-pre**) | **yes** — reddens the substance assertion alone |

So the honest bound is: **C7 pins that the clear does not escape the transaction, and cannot tell whether it rides the content statement or a sibling statement within it.** That is a stronger claim than the one it replaces and a narrower one than the row originally implied, and it is the one the measurements support.

**Why the assertion had to change too: absence is not an observation of a value.** The row originally asserted `live.Content Is.Null` after the failed upload. `Is.Null` *looks* like reading a stored value and is not — it reads the **absence** of one, and absence is the default state of the world. Rolled back, never written, never attempted, seed silently failed, method threw early: every one of those produces the identical null. A value has a cause; an absence has an entire class of causes, including *“the program under test never ran”*. The remedy seeds content `v1` before the failing upload and asserts `v1` **survives** — a positive observation with exactly one cause, which a real relocation would destroy.

**And this records a claim that had no business living outside the document.** When C8 was corrected, the architect told the operator — in a return message, not here — that C8 was *the only* row whose prediction required reasoning about a mutated program, grouping C1–C5 and C7 as predicting *“absence or persistence of a stored value, observed directly”*. **C7 was in that list, and C7 is the row that failed.** The claim was load-bearing — it was an assurance about the whole table's soundness — and it sat where no reviewer would meet it: a return message is read once and is not re-read when the thing it describes changes. It belongs here, wrong-and-corrected, which is why it is written out in full rather than summarised. Two things were wrong, and the second is the load-bearing one:

1. ***“Absence or persistence”* is not one category.** They are opposites in evidential weight, per the paragraph above, and writing them as a pair is what let an absence assertion pass as a value assertion. Agreed with QA's reading, and it was a real cause.
2. **But the sufficient cause was sorting rows by their *assertion* when the defect lives in their *falsifier*.** Every row has a mutation attached, and a mutation that was reasoned rather than executed carries the C8 risk no matter how direct the assertion it is aimed at. C7's assertion looked solid — which told me nothing whatsoever about whether its mutation had ever been run. **The question that separates a coverage row from a wish is not “what does the assertion observe?” but “has anyone executed the mutation and watched it go red?”**

*The rest of the claim survives, and that is measured rather than assumed:* QA independently built a mutation for the one other candidate row she identified (C3's differential claim) and confirmed **C3 red, C2 green, nothing else** — so apart from C7 the table's rows do discriminate as stated.
**And the defect shape is the familiar one.** The premise was read off the compliant path — *“the gate runs first, so no conversion question arises”* — and then extrapolated to the mutated path, where removing the gate is precisely what makes the conversion question arise. A predicted observable is a claim about code that does not exist yet; it has to be traced, not inferred from the code that does.
...
| §17.8's C7 row | **wrong three times; corrected 2026-09-05 by changing the rule rather than the prediction.** Round 1's mutation was an *equivalent mutant* (not the deadlock it was written up as — those 12 minutes were HTTP tests in the same filter). Round 2's hoist either escaped before any assertion ran or hung at a 10-minute timeout. The cell now carries only **M12** and **R-pre**, both executed with their output quoted, under a standing rule that the cell may name nothing else. The assertion moved from `Is.Null` to a seeded value surviving — proved on both sides, since M12 *passes* against the old form. And the bounded claim was **strengthened**: R-pre isolates the substance half, which two earlier revisions said nothing could |
| §17.8's C8 row, and §17.2's single-source inventory | **corrected 2026-09-05 during PR 3 implementation.** C8 predicted the `[AllowPatch]` mutation would return 200; measured, it returns 500, because removing the gate lets execution reach a string→`byte[]` conversion that faults and is mapped by none of the eight registered handlers. The guard is unaffected — it asserts 400 — but the row's reasoning had implied the gate was the only barrier, and the accidental second barrier does **not** generalise to string-typed properties. §17.2 gained the implementer's independent re-derivation of the 17-site / 2-assignment inventory, so the list the whole change rests on is no longer a single-source claim |
