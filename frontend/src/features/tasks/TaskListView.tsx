...
export function TaskListView({ projectId }: TaskListViewProps) {
  ...
  return (
    <div className="flex flex-col gap-4">
      {/* Toolbar: status filter + new task button */}
      ...
      {/* Topology-empty signal: distinguish "no tasks" from generic empty state.
          Shown instead of the table when we have a resolved-empty result.        */}
      {!isError && !isLoading && data && data.result.length === 0 ? (
        ...
        /* Table — shown while loading (skeleton) and once data arrives */
        <NodeResultTable
          nodes={data?.result ?? []}
          loading={isLoading}
